...
//			cnt = 0;	f.send_pid = 0;	
	...
	{		
		...
		switch(cnt)
		{
			...
			break;
			case 6:	ANO_DT_Send_speed(pixel_flow.loc_x, pixel_flow.loc_y,pixel_flow.loc_y);//FlightData.High.bara_height);		//光流模块数据		
			break;
			case 7:	ANO_DT_Send_user_f_4( Flow_PosPid_x.desired, Flow_PosPid_x.measured, Flow_SpeedPid_x.desired, Flow_SpeedPid_x.measured , 1);  
			break;
			case 9:	ANO_DT_Send_user_f_4( Flow_PosPid_y.desired, Flow_PosPid_y.measured, Flow_SpeedPid_y.desired, Flow_SpeedPid_y.measured , 2);  
			break;
			case 10:	ANO_DT_Send_user_f_4(mv.up_y, mv.up_fix_y, mv.mid_y, mv.mid_fix_y, 3);
			break;
			case 11:	ANO_DT_Send_user_f_4(mv.fix_slope, Angle.yaw, mv.mid_fix_y_cm, mv.mid_y_cm, 4);
			break;
			case 12:	ANO_DT_Send_user_f_4(pidYaw.desired, pidYaw.measured, pidHeightHigh.desired, vl53lxx.distance, 5);
//			break;
//			case 7:	ANO_DT_Send_user_f_4(mini.flow_Height_fixed,mini.flow_High, mini.flow_Height_fixed-mini.flow_High, pidHeightRate.measured, 4);
...
void ANO_DT_Send_user_f(float user_data, u8 user_id)
{
	u8 _cnt=0;
	float temp;
	u8 sum = 0;
	...
	data_to_send[_cnt++]=0xAA;
	data_to_send[_cnt++]=0xAA;
	data_to_send[_cnt++]=0xF0 + user_id;
	data_to_send[_cnt++]=0;
...
void ANO_DT_Send_user_f_4(float user_data1, float user_data2, float user_data3, float user_data4, u8 user_id)
{
	u8 _cnt=0;
	float temp;
	u8 sum = 0;
