...
send_pid:
		...
		case ANTO_MPU_MAGIC:     //发送MPU6050和磁力计的数据
			memcpy(&Anto[2],(int8_t*)&MPU6050,sizeof(_st_Mpu));//MPU6050结构体
			memcpy(&Anto[8],(int8_t*)&AK8975,sizeof(_st_Mag));//AK8975结构体
			len = 18;
			break;
		case ANTO_SENSER2:	
...
void ANO_DT_Data_Receive_Anl(u8 *data_buf,u8 num)
{
 ...
 //           Sort_PID_Flag=2;
		 ...
		 {	
				...
				Remote.thr = (vs16)(*(data_buf+4)<<8)|*(data_buf+5) ;	//油门
			  LIMIT(Remote.thr,1000,2000);		//限制在1000-2000，下同
				Remote.yaw = (vs16)(*(data_buf+6)<<8)|*(data_buf+7) ;			//偏航角，水平旋转方向
				LIMIT(Remote.yaw,1000,2000);
				Remote.roll = (vs16)(*(data_buf+8)<<8)|*(data_buf+9) ;		//翻滚角，左右
			  LIMIT(Remote.roll,1000,2000);
				Remote.pitch = (vs16)(*(data_buf+10)<<8)|*(data_buf+11) ;	//俯仰角，前后
