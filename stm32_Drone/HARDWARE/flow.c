...
void Flow_Receive1(u8 data) //串口1解析光流模块数据
{
	...
	switch(state)
	{
	 ...
	 cpi = ((Control_high*0.01f) / 11.914f) *2.54f ;
					
					
					
					mini.flow_x_i += mini.flow_x *cpi ;       
					mini.flow_y_i += mini.flow_y *cpi;  
					 	
					mini.ang_x = -10*Angle.pitch *cpi;
					mini.ang_y = -10*Angle.roll *cpi;
					...
					//高度角度融合
//					mini.flow_Height_fixed = mini.flow_High/sqrtf(((tan(Angle.pitch*ANG_2_RAD)*tan(Angle.pitch*ANG_2_RAD)+1/(cos(Angle.roll*ANG_2_RAD)*cos(Angle.roll*ANG_2_RAD)))));					
...
//	float high_temp;
	...
	////////////////////////*积分位移处理*////////////////////////////
	//低通滤波
	pixel_flow.fix_x_i += ((mini.flow_x_i - pixel_flow.fix_x_i) *0.2);// *cpi;
	pixel_flow.fix_y_i += ((mini.flow_y_i - pixel_flow.fix_y_i) *0.2);// *cpi;
	...
	//pixel_flow.ang_x += ( -10*Angle.pitch *cpi - pixel_flow.ang_x) *0.2;// *cpi;
	//pixel_flow.ang_y += ( -10*Angle.roll *cpi - pixel_flow.ang_y) *0.2;// *cpi;
	pixel_flow.ang_x += ((mini.ang_x - pixel_flow.ang_x) *0.2);// *cpi;
	pixel_flow.ang_y += ((mini.ang_y - pixel_flow.ang_y) *0.2);// *cpi;	
	...
	//式中HIGH为实际高度，单位：米
	//	 cpi = ((Control_high*0.01f) / 11.914f) *2.54f ;//写在上面了
