...
static volatile int16_t *pMpu_nf = (int16_t *)&MPU6050_nf; 
...
#define  Gyro_Read() IIC_read_Bytes(MPU6050_ADDRESS, 0X3B,buffer,6)				//Acc
#define  Acc_Read() IIC_read_Bytes(MPU6050_ADDRESS, 0x43,&buffer[6],6)		//gyro
...
void MpuGetData(void) //读取陀螺仪数据加滤波
{
    ...
    uint8_t buffer[12];
	  ...
	  Acc_Read();
		...
		if(i > 2)//gyroX gyroY gyroZ
		{	
			uint8_t k=i-3;
			const float factor = 0.15f;  //滤波因素			
			static float tBuff[3];		

			pMpu[i] = tBuff[k] = tBuff[k] * (1 - factor) + pMpu[i] * factor;   //一阶IIR滤波             
		}
	}
}
